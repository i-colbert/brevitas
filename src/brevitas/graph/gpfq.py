    ...
    def single_layer_update(self):
        ...
        for t in range(weight.shape[-1]):
            for group_index in range(self.groups):
                ...
                q_max = s * torch.clamp_min(T - z - 0.5, 0.)
                q_arg = q_arg.sign() * torch.clamp_max(q_arg.abs(), q_max[group_index, :])
                weight[group_index, :, permutation_list[group_index][t]] = q_arg
            q = self.get_quant_weights(t, 0, permutation_list)
...
class gpfq_mode(gpxq_mode):
    ...
    def __init__(
            ...
            a2gpfq_class: Optional[nn.Module] = GPFA2Q,
            a2gpfq_filter_callback: Optional[Callable[[nn.Module],
        ...
        # GPFA2Q params
        self.accumulator_bit_width = accumulator_bit_width
        self.a2gpfq_class = a2gpfq_class
        self.a2gpfq_layer = a2gpfq_filter_callback  # returns true when to use GPFA2Q
    ...
    def initialize_module_optimizer(
        ...
        if self.a2gpfq_layer(layer) and self.a2gpfq_class is not None:
            return self.a2gpfq_class(
                layer=layer,
                name=name,
                act_order=act_order,
                len_parallel_layers=len_parallel_layers,
