...
class Qronos(GPFQ):
    """
    Implementation of the Qronos algorithm as proposed in: https://arxiv.org/pdf/2505.11695
    """
    ...
    def __init__(
        ...
        super().__init__(
            layer, name, act_order, len_parallel_layers, create_weight_orig)
    ...
    def update_batch(self, module, input, current_layer):
        ...
        # NOTE: in the qronos_mode context manager, we first collect quant inputs, then
                ...
                for group_index in range(self.groups):
                    ...
                    weight[group_index, :, perm[i1:i2][i:]] -= (
                        error.unsqueeze(1).matmul(
                            h_inv_block[group_index, i, i:].unsqueeze(0))).to(dtype)
            # correct error outside the block
            for group_index in range(self.groups):
                perm = permutation_list[group_index]
                weight[group_index, :, perm[i2:]] -= (
                    error_block[group_index].matmul(
                        self.L[group_index, i1-1:i2-1, i2-1:])).to(dtype)
